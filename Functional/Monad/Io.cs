...
namespace Functional.Monad
{
    ...
    {
        public static Io<Unit> Put(string s)
        {
            return Io<Unit>.Apply(() => Console.Write(s));
        }

        public static Io<Unit> PutLine(string s)
        {
        ...
        {
            return Io<string>.Apply(() => Console.ReadLine());
        }

        public static Io<Unit> IoUnit()
        {
            return Io<Unit>.Apply(() => Unit.Only);
        }
        ...
        public static Io<TSelect> SelectMany<TValue, TResult, TSelect>(this Io<TValue> io, Func<TValue, Io<TResult>> f,
            Func<TValue, TResult, TSelect> selector)
        {
            return io.SelectMany(a => f(a).SelectMany(b => Io<TSelect>.Apply(() => selector(a, b))));
        }
    }
}
